import { useEffect, useState } from 'react';

const endpoint = `https://${process.env.REACT_APP_CODESPACE_NAME}-8000.app.github.dev/api/activities/`;

function Activities() {
  const [items, setItems] = useState([]);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadActivities = async () => {
      try {
        console.log('Activities endpoint:', endpoint);
        const response = await fetch(endpoint);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
        const data = await response.json();
        console.log('Activities fetched data:', data);
        const normalized = Array.isArray(data) ? data : (Array.isArray(data?.results) ? data.results : []);
        setItems(normalized);
      } catch (err) {
        console.error('Activities fetch error:', err);
        setError('Could not load activities.');
      } finally {
        setLoading(false);
      }
    };
    loadActivities();
  }, []);

  return (
    <div className="card section-card mb-4">
      <div className="card-header d-flex justify-content-between align-items-center">
        <span>🏃 Activities</span>
        <span className="badge bg-light text-dark">{items.length} records</span>
      </div>
      <div className="card-body p-0">
        {error && <div className="alert alert-danger error-alert m-3">{error}</div>}
        {loading && <div className="text-center py-4"><div className="spinner-border text-danger" role="status"><span className="visually-hidden">Loading...</span></div></div>}
        {!loading && !error && (
          <div className="table-responsive">
            <table className="table table-striped table-hover mb-0">
              <thead>
                <tr>
                  <th>#</th>
                  <th>User</th>
                  <th>Type</th>
                  <th>Duration (min)</th>
                  <th>Calories</th>
                  <th>Date</th>
                </tr>
              </thead>
              <tbody>
                {items.length === 0 ? (
                  <tr><td colSpan={6} className="text-center text-muted py-4">No activities found.</td></tr>
                ) : items.map((item, index) => (
                  <tr key={item.id ?? index}>
                    <td>{index + 1}</td>
                    <td>{item.user ?? item.username ?? '—'}</td>
                    <td><span className="badge badge-activity bg-primary">{item.type ?? '—'}</span></td>
                    <td>{item.duration ?? '—'}</td>
                    <td>{item.calories ?? '—'}</td>
                    <td>{item.date ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </div>
  );
}

export default Activities;
